...
if(trim($domain) != "" and sqlStringClear($domain) != "" and $domain<>"Domain veya Anahtar Kelime"){
	/**	13/09/2012 kemal.genis@turkticaret.net	**/
	$domain = sqlStringClear($domain);
	
	$keywords = explode(" ", $domain);
	...
	if(is_array($keywords))
	{
		foreach($keywords as $keyword)
		{
			...
			{
				if($kosulx == "")
					$kosulx .= " domain.name like '%".$keyword."%' ";
				else	
					$kosulx .= " and domain.name like '%".$keyword."%' ";
			}
		}
	}
	else
...
if(!isset($_SESSION['net_users']['id'])) 
	if($kosul==" where ")
		$kosul .=" offer=1 ";
	else
		$kosul .=" and offer=1 ";
