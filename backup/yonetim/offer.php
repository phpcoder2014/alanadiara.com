...
$center= new XTemplate('temp/offer.tpl');


$db=new dbMysql();

$center->assign("status", "(Bekleyen Teklifler)");
...
$query="select * from offers where status=0 and (time = ".$pastWeek." or time > ".$pastWeek.") ";
...
if($num>0) {
    ...
    foreach ($row as $rows) {
        ...
        $user=$db->get_row("select * from user where id='".$rows->customer_id."'");
