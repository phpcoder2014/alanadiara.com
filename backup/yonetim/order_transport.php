...
$db=new dbMysql();
$center->assign("status", "(Aktif Domainler)");
...
if($num>0) {
    ...
    foreach ($row as $rows) {
        ...
        if($rows->payment_type=="ccard"){
            $ccno=$db->get_row("select ff_digits,lf_digits from vpos where orderin_id='".$rows->id."'");
            $ckart="<br>".$ccno->ff_digits."****".$ccno->lf_digits;
        }else {
            $ckart="";
        }
        $center->assign("tip", $rows->payment_type.$ckart);

        $user=$db->get_row("select * from user where id='".$rows->costumer_id."'");
        $center->assign("costumer_id", $user->name."<br/>".$user->email);
        $domID=unserialize($rows->domainID);
        $domNAME="";
        foreach ($domID as $valu) {
            $domNAME .=getDomainName($valu);
        }
