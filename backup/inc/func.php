...
function userSendReqCode($param) {
    ...
    $metin .="<p>Hizmetlerimize göstermiş olduğunuz ilgi için teşekkür ederiz.<br>alanadiara.com Takımı </p>";
	 ...
	 $result = sendMail($metin,$param['email'],$param['name'],"Yeni Başvuru");
	 return $result;
}
...
function sendMail($metin,$email,$name,$mesaj_name="alanadiara.com",$sendTeknik="false") {
    ...
    $mail = new PHPMailer();
      $mail->Host		= "mx8.turkticaret.net"; //"89.106.12.24"; // SMTP server
      $mail->Port		= 587;
      $mail->Mailer  	= "smtp";
      $mail->SMTPAuth = true;
      $mail->Username = "info@alanadiara.com";
      $mail->Password = "Deneme.11"; //"TAZm421";
...
function sqlStringClear($word)
{
	$bad = array("+",'%', "!",'?','#','@','<','>', '|','~', ",", '"', "'", "$", "&", "/",'\\', '*', "(", ")", "ç", "ğ", "ı", "ö", "ş", "ü", "ã§", "äÿ","ä±","ã¶","åÿ","ã¼");
	$god = array(" "," ", " "," ",' ',' ',' ',' ', ' ',' ', " ", " ", " ", " ", " ", " ",' ',  ' ', " ", " ", "c", "g", "i", "o", "s", "u", "c" , "g" ,"i" ,"o" , "s","u");
	...
	//$word = mb_convert_encoding($word, "UTF-8",mb_internal_encoding());	// Apache2Triad localde problem çıkarsa açılmalı
	$word = str_replace($bad, $god, $word);
